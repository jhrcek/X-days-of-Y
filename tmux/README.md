...
`https://<user>.github.io/X-days-of-Y/tmux/`, and the repo-root `index.html` (generated by
`cabal run build-site`) links to it.
...
```sh
cabal run tmux-course     # just this course
...
| Path | What it is |
| --- | --- |
| `src/Courses/Tmux.hs` | the `Course` record: title, cover copy, config-file metadata, the day list |
| `src/Course/Day/D01.hs` … `D14.hs` | the actual content, one `Day` value per module |
| `../course-builder/src/Course/Types.hs` | the shape of a course and of one lesson |
| `../course-builder/src/Course/Markup.hs` | the vocabulary lessons are written in (`k`, `c`, `termWin`, `gotcha`, …) |
| `../course-builder/src/Course/Render.hs` | the page templates |
| `../course-builder/assets/style.css` | the whole design, shared by every course |

`assets/style.css` here is a **generated copy** of the shared stylesheet — edit the one in
`course-builder/`. To change a lesson, edit its `DNN.hs` and re-run the build. The cumulative reference, the
