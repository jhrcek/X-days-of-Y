    ...
    Day
        ...
        , dayDrills =
            [ "Create three sessions in the background without leaving the one you are in: "
                <> c "tmux new -d -s api -c ~/src/api"
                <> " and two more for real projects."
            , "Move between them with "
                <> k "C-b s"
                <> ". Then learn the fast path: "
                <> k "C-b L"
                <> " to bounce between the last two, "
                <> k "C-b ("
                <> " and "
                <> k "C-b )"
                <> " to walk the list."
            ...
            , "Attach a second terminal to the same session and watch the window shrink to the \
              \smaller terminal's size. Then set "
                <> c "set -wg aggressive-resize on"
                <> " and \
                   \see what changes."
            , "Evict the other client: from one terminal run "
                <> c "tmux attach -d -t <session>"
                <> ". Check "
                <> c "tmux lsc"
                <> " before and after."
            , "Write the wrapper: a two-line shell function "
                <> c "tm"
                <> " that does "
                <> c "tmux new -A -s \"$1\" -c \"$PWD\""
                <> ". Use it for the rest of the course."
            , "Deliberately destroy a session you are attached to ("
                <> c "tmux kill-session -t x"
                <> " from another terminal) and watch what your client does. Then set "
                <> opt "detach-on-destroy"
                <> " to "
                <> c "off"
                <> " and do it again."
            ]
        , dayQuiz =
            ...
            ,
                ...
                , do
                    ...
                    p_
                        "The practical test: when you finish with something, do you want to close four \
            ...
            ,
                ( "You run "
                    ...
                    <> c "duplicate session"
                    <> " half the time. What is the fix, and why is it not "
                    <> c "kill-session"
                    <> "?"
                , p_ $ do
                    c "tmux new -A -s api"
...
body fig = do
    ...
    block "Moving between them" $ do
        defs
            [
                ( k "C-b s"
                , "The session tree. Sessions collapsed or expanded, previews on the right, \
                  \a shortcut letter on every line. "
                    <> k "x"
                    <> " kills, "
                    <> k "t"
                    <> " tags, "
            ...
            ,
                ( k "C-b L"
                , "The previous session. The one you will actually use, for the same reason "
                    <> k "C-b l"
                    <> " beat "
                    <> k "C-b n"
                    <> " on Day 2."
            ...
            ,
                ( k "C-b (" <> " / " <> k "C-b )"
                , "Previous and next session in order. Fine with three \
    ...
    block "Ending a session, on purpose and by accident" $ do
        ...
        defs
            [
                ( opt "detach-on-destroy"
                , "On by default: when your session is destroyed, your client \
                  \is detached and you land back in the shell. Set it to "
                    <> c "off"
                    <> " and the \
            ...
            ,
                ( opt "destroy-unattached"
                , "Off by default: sessions survive with nobody watching. \
