...
day =
  Day
    ...
    , dayConfig = []
    , dayDrills =
        [ "Split a window three ways with " <> k "C-b %" <> " and " <> k "C-b \"" <> ". Move \
        ...
        , "Now do real work in the smallest pane, and when it becomes annoying press "
            <> k "C-b z" <> ". Work. Press it again. This is the single highest-value key on \
            \the page — most people resize when they should zoom."
        , "Press " <> k "C-b Space" <> " six times slowly and watch the seven presets go past. \
          \Then set one directly with " <> k "C-b M-4" <> " and adjust " <> opt "main-pane-width"
            <> " from the command prompt: " <> c "set -w main-pane-width 60%" <> "."
        , "Hold " <> k "C-b" <> " then tap " <> k "C-Left" <> " several times without \
          \re-pressing the prefix. That works because the resize keys are bound with "
            <> c "-r" <> " (repeatable) — Day 8 explains the mechanism."
        , "Break the layout on purpose by dragging panes to silly sizes, then repair it with "
            <> k "C-b E" <> ", and undo that with " <> c "select-layout -o" <> "."
        , "Run " <> c "tmux lsp" <> " and read the output: index, size, command, and which pane \
          \is active. Compare it with " <> k "C-b q" <> "."
        , "Try the floating pane: " <> k "C-b *" <> ". Note that it hovers above the layout \
          \instead of taking space from it, and that " <> k "C-b x" <> " closes it."
        ]
    , dayQuiz =
        [ ( "Why does " <> k "C-b %" <> " make a left/right split when its command is "
              <> c "split-window -h" <> ", and " <> c "h" <> " usually means horizontal?"
          , do
              p_ $ do
        ...
        , ( "What is the practical difference between " <> c "split-window -h" <> " and "
              <> c "split-window -fh" <> "?"
          , p_ $ do
              "Without "
              c "-f"
        ...
        , ( "What does " <> c "layout: bb62,159x48,0,0{79x48,0,0,79x48,80,0}" <> " in "
              <> c "tmux lsw" <> " output mean, and what is it for?"
          , p_ $ do
...
body fig = do
  ...
  block "Getting around, and getting out of the way" $ do
    ...
    tip $ do
      ...
      p_ "This dissolves the usual complaint that splits are too small. Keep a comfortable, \
