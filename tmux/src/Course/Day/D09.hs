...
day =
    Day
        ...
        , dayDrills =
            [ "Run "
                <> c "tmux display -p -a | less"
                <> " and read the whole list once. It is \
        ...
        , dayQuiz =
            ...
            ,
                ( "Why does "
                    <> c "#{?session_attached,yes,no}"
                    <> " work, but "
                    <> c "#{?#{session_attached},yes,no}"
                    <> " is what you see people write for other \
                       \things?"
                , p_ $ do
                    "The condition of "
                    c "?"
...
body fig = do
    ...
    block "String surgery" $ do
        p_
            "Modifiers go before the variable, separated by a colon. They compose, separated by \
            \semicolons."
        defs
            ...
            ,
                ( c "#{t/f/%H#:%M:window_activity}"
                , "a custom strftime format — note the "
                    <> c "#:"
                    <> ", because a bare colon would end the modifier"
                )
            ,
                ( c "#{s/old/new/:window_name}"
                , "substitute throughout; the pattern is a regular \
                  \expression, and a trailing "
                    <> c "i"
                    <> " ignores case"
                )
            ,
                ( c "#{q:pane_current_path}"
                , "shell-quote the value — essential when a format is \
                  \interpolated into a command"
                )
            ,
                ( c "#{E:status-left}"
                , "expand the value of an option, rather than its name; "
                    <> c "#{T:…}"
                    <> " also runs it through strftime"
                )
