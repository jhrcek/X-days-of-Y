...
day =
    Day
        ...
        , dayDrills =
            [ "Add today's config and try each binding in turn. The popup shell ("
                <> k "C-b C-g"
                <> ") is the one that changes daily habits: a throwaway command without disturbing \
                   \your layout."
            , "Watch hooks fire. Set "
                <> c "set-hook -g after-split-window 'display-message \"split: #{pane_index}\"'"
                <> " and split a few times. Then remove it with "
                <> c "set-hook -ug after-split-window"
                <> "."
            , "Discover which hooks exist: "
                <> c "tmux show-hooks -g | head -40"
                <> " lists the "
                <> c "after-*"
                <> " ones, and the man page's HOOKS section lists the rest. Note \
                   \that almost every command has an "
                <> c "after-"
                <> " hook."
            , "Scope a hook: set one on a single session ("
                <> c "set-hook '…'"
                <> " without "
                <> c "-g"
                <> ") and confirm it does not fire in your other sessions."
        ...
        , dayQuiz =
            ...
            ,
                ( "In "
                    <> c "command-prompt -p 'name:' \"new-session -d -s '%%'\""
                    <> ", what is "
                    <> c "%%"
            ...
            ,
                ( "You set "
                    <> c "after-select-pane"
                    <> " to run something, and tmux appears to hang \
