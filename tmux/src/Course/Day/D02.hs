...
day =
    Day
        ...
        , dayDrills =
            [ "In your "
                <> c "main"
                <> " session, build the shape of a real project: "
                <> k "C-b c"
                <> " three times, and name each one with "
                <> k "C-b ,"
                <> " — say "
                <> c "edit"
                <> ", "
            ...
            , "Move between them with "
                ...
                <> k "C-b 2"
                <> " for five minutes of \
                   \real work. Then stop using the numbers and use "
                <> k "C-b l"
                <> " to bounce \
                   \between the last two. Notice how much of your switching is really just bouncing."
            , "Open "
                <> k "C-b w"
                <> ". Navigate with the arrows, watch the preview pane, choose \
                   \with Enter. Press "
                <> k "q"
                <> " to escape without choosing."
            , "Start "
                <> c "top"
                <> " in a window you have not renamed, and watch the window name \
                   \change by itself. Now "
                <> k "C-b ,"
                <> " it to something and start "
                <> c "top"
                <> " again — the name stays. You have just turned "
                <> opt "automatic-rename"
                <> " off for that one window."
            , "Create a window straight into a directory: "
                <> k "C-b :"
                <> " then "
            ...
            , "Reorder: "
                <> k "C-b ."
                <> " and give an index that is already taken. Read the error. \
                   \Now try an index that is free."
            , "Kill one with "
                <> k "C-b &"
                <> " and confirm. Then close another by just typing "
                <> c "exit"
                <> " in its shell. Same result, different route."
            ]
        , dayQuiz =
            [
            ...
            ,
                ( "You have twelve windows and want number 11. "
                    <> k "C-b 1"
                    <> k "1"
            ...
            ,
                ...
                , do
                    p_
                        "Window 3 is named build and is the last window you were in. Window 4 is \
...
body fig = do
    block "Tabs, but they outlive the terminal" $ do
        ...
        p_ $ do
            ...
            b_
                "one session per project, one window per concern, panes only when you genuinely need \
    ...
    block "Getting around" $ do
        ...
        defs
            [
                ( k "C-b l"
                , "The previous window. Most switching is really alternation between two \
                  \things — the editor and the thing you just ran. Learn this one first."
                )
            ,
                ( k "C-b 0" <> " … " <> k "C-b 9"
                , "Absolute jump by index. Fast and unambiguous, as \
                  \long as your important windows live at low indices."
                )
            ,
                ( k "C-b n" <> " / " <> k "C-b p"
                , "Next and previous by index, wrapping at the ends. \
                  \Good for a sweep, bad for a target."
                )
            ,
                ( k "C-b w"
                , "The interactive picker: a tree of sessions, windows and panes with a live \
                  \preview of each. Arrows to move, Enter to choose, "
                    <> k "q"
                    <> " to \
