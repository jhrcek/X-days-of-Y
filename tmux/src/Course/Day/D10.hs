  ...
  Day
    ...
    , dayDrills =
        [ "Before changing anything, read the default: "
            <> c "tmux show -g status-format" <> ". That single option is the whole line, and \
            \every other status option is interpolated into it."
        , "Discover the length trap: set " <> c "status-left" <> " to something 30 characters \
          \long and watch it get cut at 10. Then raise " <> opt "status-left-length" <> "."
        , "Move it: " <> c "tmux set -g status-position top" <> ". Live with it for a day — \
        ...
        , "Add today's config, reload, and then take it apart: change one colour, "
            <> k "C-b R" <> ", look. The feedback loop is under a second, so experiment rather \
            \than plan."
        , "Turn on monitoring: leave a long build in one window, switch away, and watch the "
            <> c "#" <> " flag appear. Then jump straight to it with " <> k "C-b M-n" <> "."
        ...
        , "Add something live to the right: " <> c "#(uptime | sed 's/.*load average: //')"
            <> " with " <> c "status-interval 5" <> ". Then decide whether you actually want a \
    ...
    , dayQuiz =
        ...
        , ( "What is the difference between " <> opt "status-style" <> " and a " <> c "#[…]"
              <> " directive?"
          , p_ $ do
              opt "status-style"
        ...
        , ( "You set " <> opt "monitor-activity" <> " on and now every window is permanently \
...
body fig = do
  ...
  block "Alerts: making windows speak up" $ do
    ...
    defs
      [ (opt "monitor-activity", "any output at all. Flag " <> c "#" <> ". Useful, and noisy \
            \if a window is tailing a log.")
      , (opt "monitor-bell", "the program rang the terminal bell. Flag " <> c "!" <> ". This is \
            \what a shell does when a long command finishes, if you configure it to.")
      , (opt "monitor-silence 30", "no output for 30 seconds. Flag " <> c "~" <> ". The one for \
