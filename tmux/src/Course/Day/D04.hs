...
day =
    Day
        ...
        , dayConfig = []
        , dayDrills =
            [ "Run something with a lot of output ("
                <> c "find /usr -type f | head -5000"
                <> "). \
                   \Enter copy mode with "
                <> k "C-b ["
                <> " and look at the top right corner: that is \
            ...
            , "Select three lines and copy them: start a selection, move, then finish with "
                ...
                <> " (vi) or "
                <> k "M-w"
                <> " (emacs). Paste them into another pane \
                   \with "
                <> k "C-b ]"
                <> "."
            , "Do it again with a rectangle: turn on block selection ("
                <> k "v"
                <> " in vi, "
                <> k "R"
                <> " in emacs) and grab a column out of some tabular output."
            , "Copy three different things in a row, then press "
                <> k "C-b #"
                <> " and "
                <> k "C-b ="
                <> ". Buffers are a stack, not a single clipboard."
            , "Capture without copy mode: "
                <> c "tmux capture-pane -p -S - > /tmp/pane.txt"
        ...
        , dayQuiz =
            [
                ( "You press "
                    <> k "C-b ["
                    <> ", scroll up, and now your usual keys do strange \
            ...
            ,
                ...
                , do
                    p_
                        "Because the copy landed in a tmux paste buffer on the remote server, which \
...
body fig = do
    ...
    block "Buffers are a stack" $ do
        ...
        defs
            ...
            ,
                ( k "C-b ="
                , "pick one interactively — with a preview, and "
                    <> k "d"
                    <> " to delete, "
                    <> k "e"
