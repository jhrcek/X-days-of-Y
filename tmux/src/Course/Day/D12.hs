  ...
  Day
    ...
    , dayDrills =
        [ "Break a pane out and put it back. Split a window, run something in the new pane, "
            <> k "C-b !" <> " to break it out, then " <> k "C-b m" <> " on it, go back, and "
            <> k "C-b @" <> " to pull it in. The process never restarts."
        , "Move a pane across sessions: mark it, switch to another session with " <> k "C-b L"
            <> ", and " <> k "C-b @" <> ". Panes are not owned by sessions in any way that \
            \prevents this."
        , "Link a window into two sessions: " <> c "tmux link-window -s api:logs -t infra:"
            <> ". Change something in it from one session and watch the other. Then "
            <> c "unlink-window" <> " it from one."
        , "Make some gaps in your window indices by killing windows, then close them with "
            <> k "C-b M-r" <> " (today's binding for " <> c "move-window -r" <> ")."
        , "Use " <> k "C-b f" <> " to find a window by something that is currently "
            <> i_ "on screen" <> " in it, not by its name. Very few people know this searches \
            \pane contents."
        , "Open " <> k "C-b >" <> " and " <> k "C-b <" <> ". These menus have been in tmux for \
          \years and are almost undiscoverable — read what is in them."
        , "Read the mouse: " <> c "tmux lsk -T root" <> ". Find " <> c "WheelUpPane" <> " and \
          \work out, from the binding, exactly why scrolling puts you into copy mode."
        , "Tag and act in bulk: " <> k "C-b s" <> ", tag two sessions with " <> k "t" <> ", \
          \press " <> k ":" <> " and run a command against all of them."
        ]
    , dayQuiz =
        ...
        , ( "What is the difference between " <> c "move-window" <> " and " <> c "link-window"
              <> "?"
          , p_ $ do
              c "move-window"
        ...
        , ( "Why does " <> c "join-pane" <> " with no " <> c "-s" <> " sometimes work and \
            \sometimes complain?"
          , p_ $ do
...
body fig = do
  block "Panes are not stuck where you made them" $ do
    ...
    defs
      [ (c "break-pane" <> " (" <> k "C-b !" <> ")", "this pane leaves its window and becomes a \
            \window of its own. " <> c "-d" <> " does not follow it; " <> c "-n name"
            <> " names the new window.")
      , (c "join-pane", "a pane moves into this window, splitting it. " <> c "-h" <> " / "
            <> c "-v" <> " choose the direction, " <> c "-b" <> " puts it before, "
            <> c "-l 30%" <> " sizes it. With no " <> c "-s" <> " it takes the marked pane.")
      ]
    sh
