  ...
  Day
    ...
    , dayDrills =
        [ "Write the bootstrap script for one real project of yours, following the pattern \
          \below. Put it in " <> c "~/bin" <> " and use it tomorrow morning instead of \
        ...
        , "Drive a pane from another pane. Split, note the id with "
            <> c "tmux display -p '#{pane_id}'" <> ", then from the other side: "
            <> c "tmux send-keys -t %N 'date' Enter" <> "."
        , "Compare the two ways to start a program: " <> c "tmux neww 'top'" <> " and "
            <> c "tmux neww" <> " followed by " <> c "send-keys 'top' Enter" <> ". Quit "
            <> c "top" <> " in each and note which window survives."
        , "Turn on " <> c "remain-on-exit" <> " for one pane (" <> c "tmux set -p remain-on-exit on"
            <> "), run " <> c "false" <> " in it, and read what tmux leaves behind. Bring it \
            \back with " <> c "respawn-pane -k" <> "."
        , "Log a pane to a file with today's " <> k "C-b P" <> " binding, generate some output, \
          \turn it off, and read the file."
        , "Mark a pane with " <> k "C-b m" <> ", go to another one, and use "
            <> k "C-b C-r" <> " to re-run the last command over there. This is the \
            \edit-here / run-there loop, and it is worth building the habit around."
        , "Save a layout you like: " <> c "tmux lsw -F '#{window_layout}'" <> ", then destroy \
          \the arrangement and restore it with " <> c "select-layout '<that string>'" <> "."
        ]
    , dayQuiz =
        [ ( "Why does " <> c "tmux new-window 'npm run dev'" <> " leave you with no window when \
            \the server stops, while " <> c "send-keys 'npm run dev' Enter" <> " does not?"
          , p_ $ do
        ...
        , ( "What is wrong with " <> c "tmux send-keys -t api:1 'make' Enter" <> " in a script \
        ...
        , ( "When would you use " <> c "if-shell -F" <> " rather than plain " <> c "if-shell"
              <> "?"
          , p_ $ do
...
body fig = do
  ...
  block "The pattern" $ do
    ...
    defs
      [ (c "=$SESSION", "the " <> c "=" <> " prefix demands an exact name match, so a session \
            \called " <> c "api" <> " is never confused with " <> c "api-staging" <> ".")
      , (c "-d", "everything is built detached. Without it your screen jumps between windows \
            \as the script runs, and the terminal size is whatever the last command left.")
      , (c "-P -F '#{pane_id}'", "creation commands can print what they created. Capture it in \
            \a shell variable and every later command has an unambiguous target.")
      , (c "-c \"$ROOT\"", "set the directory once per window instead of typing " <> c "cd"
            <> " into four shells.")
      ]
    tip $ p_ $ do
  ...
  block "Two ways to start a program, and they are not equivalent" $ do
    cols
      ...
      , do
          ...
          p_ [class_ "lede-sm"] "The pane runs your shell; the command is just something typed \
