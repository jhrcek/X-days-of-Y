  ...
  Day
    ...
    , dayDrills =
        ...
        , "Send a command to a pane you are not in. Split a window, note the other pane's id, \
          \then from this pane run " <> c "tmux send-keys -t %7 'echo hello' Enter" <> "."
        , "Try each target token against a session with several windows: " <> c "-t :^"
            <> ", " <> c "-t :$" <> ", " <> c "-t :!" <> ", " <> c "-t :+" <> ", " <> c "-t :-2"
            <> ". Use " <> c "tmux display -p -t ... '#{window_name}'" <> " to see where you landed."
        , "Prove the ambiguity trap: make windows named " <> c "test" <> " and " <> c "testing"
            <> ", then target " <> c "-t :test" <> " and " <> c "-t :=test" <> " and compare."
        , "Chain three commands from a shell in the two legal ways — "
            <> c "tmux neww \\; splitw -h \\; splitw -v" <> " and "
            <> c "tmux neww ';' splitw -h" <> " — and then get it wrong on purpose without the \
            \escaping to see what the shell does with it."
        , "Add a command alias: " <> c "tmux set -s command-alias[100] zoom='resize-pane -Z'"
            <> " and then run " <> c "tmux zoom" <> "."
        ]
    , dayQuiz =
        [ ( "Why does " <> c "tmux neww; splitw" <> " in a shell do something different from "
              <> c "tmux neww \\; splitw" <> "?"
          , p_ $ do
