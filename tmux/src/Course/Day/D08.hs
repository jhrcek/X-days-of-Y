...
day =
  Day
    ...
    , dayDrills =
        [ "Press " <> k "C-b /" <> " and then some key you are unsure about. tmux tells you \
          \what it runs. Do it for " <> k "C-b E" <> ", " <> k "C-b <" <> " and " <> k "C-b >"
            <> " — three defaults most people never discover."
        , "Add today's config blocks and reload with " <> k "C-b R" <> ". Then split a pane and \
          \confirm the new pane starts in the same directory."
        , "Run " <> c "tmux lsk -T copy-mode-vi | head -30" <> " and find the binding for "
            <> k "v" <> ". Then add your own: bind " <> k "Y" <> " in that table to "
            <> c "send-keys -X copy-pipe-line-and-cancel" <> "."
        , "Bind something without a prefix and feel the difference: "
            <> c "bind -n M-Enter resize-pane -Z" <> ". Then unbind it — " <> k "M-Enter"
            <> " is a real key some programs want."
        , "Make a repeat key of your own and compare: bind " <> k "C-b y" <> " to "
            <> c "next-window" <> " with and without " <> c "-r" <> ", and tap it three times."
        , "Build a two-key chord. Add the three lines from the “Modal tables” section and check \
          \that " <> k "C-b g" <> k "s" <> " opens a git status window."
        , "Give every custom binding a " <> c "-N" <> " note, then look at " <> k "C-b ?"
    ...
    , dayQuiz =
        ...
        , ( "Why can you tap " <> k "C-b" <> k "C-Left" <> k "Left" <> k "Left"
              <> " and keep resizing, but not do the same with " <> k "C-b" <> k "n" <> "?"
          , p_ $ do
        ...
        , ( "You want " <> k "C-b" <> " to reach the program in the pane. What happens, and why \
            \does " <> k "C-b" <> k "C-b" <> " work?"
          , p_ $ do
              "The first "
              k "C-b"
        ...
        , ( "How would you build a “leader key” with several keys under it, like " <> c "g"
              <> " then " <> c "s" <> " for git status?"
          , do
              p_ $ do
              ...
              p_ "Tables are how copy mode works too, so this is not a trick bolted on the \
