...
day =
    Day
        ...
        , dayDrills =
            [ "Run "
                <> c "tmux new -s day1"
                <> ". Start something long-lived in it — "
                <> c "top"
                <> ", a log tail, a dev server."
            , "Detach with "
                <> k "C-b d"
                <> ". Confirm the process is still alive from outside: "
                <> c "tmux ls"
                <> ", then "
                <> c "ps"
                <> " for it."
            , "Close the terminal window entirely. Open a new one. "
                <> c "tmux attach -t day1"
                <> ". Everything is exactly where you left it."
            , "Attach to the same session from two terminals side by side. Type in one and watch \
              \the other. Now detach one with "
                <> k "C-b d"
                <> " — the other is unaffected."
            , "Press "
                <> k "C-b ?"
                <> " and read the whole list once. You will not remember it. \
                   \Notice instead that every line is a "
                <> c "bind-key"
                <> " command."
            , "Press "
                <> k "C-b :"
                <> ", type "
                <> c "rename-session tuesday"
                <> ", press Enter. \
        ...
        , dayQuiz =
            ...
            ,
                ( "You are inside tmux running an editor that itself wants "
                    <> k "C-b"
                    <> ". How do \
                       \you feed it through?"
                , p_ $ do
            ...
            ,
                ( "Why does "
                    <> c "tmux ls"
                    <> " sometimes say “no server running”, even though you \
