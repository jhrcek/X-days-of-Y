    ...
    Day
        ...
        , dayDrills =
            [ "Reproduce the stale agent problem: attach to an old session over a new ssh \
              \connection and run "
                <> c "ssh-add -l"
                <> " in a pane that existed before. Then fix \
        ...
        , dayQuiz =
            [
                ( "You reattach to a week-old session over a fresh ssh connection and "
                    <> c "git push"
                    <> " cannot reach your ssh agent. Why does "
...
body fig = do
    block "The environment problem" $ do
        ...
        p_
            "For the shells that already exist, the fix is indirection — put a stable path in the \
