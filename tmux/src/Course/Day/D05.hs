...
day =
    Day
        ...
        , dayDrills =
            [ "Create "
                <> c "~/.tmux.conf"
                <> " with today's blocks. Then load it into a running \
                   \server: "
                <> k "C-b :"
                <> " "
                <> c "source ~/.tmux.conf"
                ...
                <> k "C-b R"
                <> " does it."
            , "Break it on purpose: add the line "
                <> c "set -g nonsense 1"
                <> " and reload. Read \
                   \where the error appears and how the rest of the file still runs."
            , "Prove the scope chain. Run "
                <> c "set -w window-style bg=#202020"
                <> " then "
                <> c "set -p window-style bg=#402020"
                <> " in one pane of a split. The pane option \
                   \wins for that pane; the window option covers the rest. Undo both with "
                <> c "set -pu window-style"
                <> " and "
                <> c "set -wu window-style"
                <> "."
            , "Compare "
                <> c "tmux show -gw"
                <> " with "
                <> c "tmux show -w"
                <> " in a window \
                   \where you have set something, then with "
                <> c "tmux show -wA"
                <> " and find the "
                <> c "*"
                <> " markers on inherited values."
            , "Open "
                <> k "C-b C"
                <> ". Navigate to an option, press "
                <> k "v"
                <> " to read its \
                   \description, "
                <> k "s"
                <> " to set it, "
                <> k "u"
                <> " to unset. This is the \
                   \fastest way to explore the several hundred options without leaving tmux."
            , "Set a user option and read it back: "
                <> c "tmux set -g @project api"
                <> " then "
                <> c "tmux show -gv @project"
                <> ". Day 9 uses these inside formats."
            , "Kill the server ("
                <> c "tmux kill-server"
                <> ") and start again, to confirm your \
        ...
        , dayQuiz =
            ...
            ,
                ( "You re-source your config after deleting a "
                    <> c "bind"
                    <> " line, but the key \
                       \still works. Why?"
                , p_ $ do
...
body fig = do
    ...
    block "Four scopes" $ do
        ...
        defs
            [
                ( "server " <> c "-s"
                , "One set for the whole server. Terminal handling, timings, \
                  \clipboard behaviour: things that are not about any particular session."
                )
            ,
                ( "session " <> c "-g" <> " or per-session"
                , "Status line, prefix key, mouse, history \
            ...
            ,
                ...
                , "Styles, cursor, "
                    ...
                    <> ", "
                    <> opt "remain-on-exit"
                    <> ". Pane options fall back to the window's value."
                )
            ]
        fig
        p_ $ do
