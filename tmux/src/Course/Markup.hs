...
-- | A @~/.tmux.conf@ excerpt shown inside the prose (the per-day \"add this\"
...
-- | A terminal window with a caption bar and no tmux status line.
termWin :: Text -> [Text] -> Html ()
termWin title body = div_ [class_ "term"] $ do
  termBar title
  pre_ [class_ "term-body"] $ code_ $ toHtml (T.unlines body)

-- | A terminal window showing a tmux status line along the bottom.
...
-- | The reasoning behind a design decision in tmux itself.
why :: Html () -> Html ()
